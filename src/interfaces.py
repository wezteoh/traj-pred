...
class AutoregressiveMultiplePathPredictionInterface(BasePredictionInterface):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        samples_original_scale = unnormalize(samples, self.data_mean, self.data_std)
        y_original_scale = unnormalize(y, self.data_mean, self.data_std)
        metric_dict = self.compute_jade_jfde(
            samples_original_scale,
            y_original_scale[
                :,
                self.hparams.interface.validation_prefix_length
                - 1 : self.hparams.interface.validation_prefix_length
        ...
        if (
            (self.trainer.current_epoch + 1) // self.hparams.interface.upload_every_n_epochs == 0
            and batch_idx == 0
            ...
            video_dir = f"{self.hparams.train.results_dir}/samples"
    ...
    def sample(self, x: torch.tensor, max_length: int, num_paths: int):
        """
        x: [b, t, num_agents, 2]
        """
        ...
        init_cls_dist = F.softmax(init_cls_out, dim=-1)
        ...
        for _ in range(num_paths):
            ...
            for t in range(max_length - 1):
                reg_out, cls_out = self.model.generate(x, inference_cache)
                cls_out = cls_out[:, -1].detach()
                selected_scene_idxs = torch.multinomial(
                    F.softmax(cls_out, dim=-1), num_samples=1
                )  # [b, 1]
                selected_scene_idxs = rearrange(selected_scene_idxs, "b 1 -> b 1 1 1 1")
                selected_scene_idxs = selected_scene_idxs.repeat(
                    1, 1, 1, reg_out.shape[-2], reg_out.shape[-1]
                )
                x = reg_out.gather(2, selected_scene_idxs).squeeze(2)  # [b, t, k, a, 2]
                preds.append(x.detach())
            samples.append(torch.cat(preds, dim=1))  # [b, t, a, 2]
        return torch.stack(samples, dim=1)  # [b, num_paths, t, a, 2]
    ...
    def test_step(self, batch, batch_idx):
        ...
        metric_dict = self.compute_jade_jfde(
            ...
            y_original_scale[
                ...
                - 1 : self.hparams.test.prefix_length
