...
def tune(config):
    ...
    interface.hparams.optimizer.lr_schedule = None
    ...
    lr_finder = tuner.lr_find(
        ...
        min_lr=1e-4,
        max_lr=1.0,
        num_training=100,  # ~batches to sweep
    )
    fig = lr_finder.plot(suggest=True)
    Path(config.train.results_dir).mkdir(parents=True, exist_ok=True)
    fig.savefig(Path(config.train.results_dir) / "lr_finder.png")
